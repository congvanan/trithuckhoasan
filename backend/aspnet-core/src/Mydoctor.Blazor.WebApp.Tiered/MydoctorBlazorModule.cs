...
using Mydoctor.Blazor.WebApp.Tiered.Client;
using Mydoctor.Blazor.WebApp.Tiered.Client.Menus;
using Mydoctor.Blazor.WebApp.Tiered.Components;
using Mydoctor.Blazor.WebApp.Tiered.Menus;
...
[DependsOn(
    typeof(MydoctorHttpApiClientModule),
...
public class MydoctorBlazorModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(MydoctorBlazorModule).Assembly
            );
        });
        ...
        Configure<AbpBundlingOptions>(options =>
        {
            ...
            options.StyleBundles.Configure(
                ...
                bundle =>
                {
                    ...
                    bundle.AddFiles(new BundleFile("/Mydoctor.Blazor.WebApp.Tiered.Client.styles.css", true));
                }
            );
        });
    }
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<MydoctorBlazorModule>(hostingEnvironment.ContentRootPath);
            });
        }
    }
    ...
    private void ConfigureMenu(IConfiguration configuration)
    {
        ...
        {
            options.MenuContributors.Add(new MydoctorMenuContributor(configuration));
        });

        Configure<AbpToolbarOptions>(options =>
        {
            options.Contributors.Add(new MydoctorToolbarContributor());
        });
    }
    ...
    private void ConfigureRouter(ServiceConfigurationContext context)
    {
        ...
        {
            options.AppAssembly = typeof(MydoctorBlazorModule).Assembly;
            options.AdditionalAssemblies.Add(typeof(MydoctorBlazorClientModule).Assembly);
        });
    }
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("Mydoctor");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]!);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "Mydoctor-Protection-Keys");
        }
    }
