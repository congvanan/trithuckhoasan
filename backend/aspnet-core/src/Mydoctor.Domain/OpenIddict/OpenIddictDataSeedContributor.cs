...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Mydoctor") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Mydoctor", DisplayName = "Mydoctor API", Resources = { "Mydoctor" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["Mydoctor_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Mydoctor_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // ABP React Next.js Client
        var reactClientId = configurationSection["AbpReact_Next_App:ClientId"];
        if (!reactClientId.IsNullOrWhiteSpace())
        {
            var reactRootUrl = configurationSection["AbpReact_Next_App:RootUrl"]?.TrimEnd('/');

            await CreateApplicationAsync(
                name: reactClientId!,
                type: OpenIddictConstants.ClientTypes.Public,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "ABP React Next.js Application",
                secret: null,
                ...
                scopes: commonScopes,
                redirectUri: $"{reactRootUrl}/auth/openiddict",
